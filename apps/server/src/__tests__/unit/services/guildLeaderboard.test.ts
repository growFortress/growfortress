...
describe('Guild Leaderboard Service', () => {
  ...
  describe('getWeeklyLeaderboard', () => {
    it('returns cached leaderboard when available', async () => {
      const cachedData = {
        entries: [createMockLeaderboardEntry(), createMockLeaderboardEntry({ rank: 2, guildId: 'guild-456' })],
        total: 2,
      };
    ...
    it('fetches from database when cache miss', async () => {
      ...
      const guilds = [
        createMockGuild({ id: 'guild-1', name: 'Guild 1', honor: 1500, _count: { members: 10 } }),
        createMockGuild({ id: 'guild-2', name: 'Guild 2', honor: 1200, _count: { members: 8 } }),
      ];
      ...
      expect(mockPrisma.guild.findMany).toHaveBeenCalled();
      expect(result.entries).toHaveLength(2);
    ...
    it('calculates battle stats correctly', async () => {
      ...
      const battleStats = [
        { attackerGuildId: 'guild-1', defenderGuildId: 'guild-2', winnerGuildId: 'guild-1' },
        { attackerGuildId: 'guild-1', defenderGuildId: 'guild-3', winnerGuildId: 'guild-1' },
        { attackerGuildId: 'guild-2', defenderGuildId: 'guild-1', winnerGuildId: 'guild-2' },
      ];
  ...
  describe('getMemberContributions', () => {
    it('returns member contributions sorted by XP', async () => {
      const members = [
        {
          ...createMockGuildMember({ userId: 'user-1', weeklyXpContributed: 500 }),
          user: { displayName: 'Top Contributor' },
        },
        {
          ...createMockGuildMember({ userId: 'user-2', weeklyXpContributed: 200 }),
  ...
  describe('snapshotWeeklyRankings', () => {
    ...
    it('calculates battle wins from groupBy results', async () => {
      const guilds = [createMockGuild({ id: 'guild-1', _count: { members: 10 } })];
  ...
  describe('distributeWeeklyRewards', () => {
    ...
    it('distributes rewards to top 25 guilds', async () => {
      const guilds = Array.from({ length: 30 }, (_, i) =>
        createMockGuild({ id: `guild-${i}`, honor: 1500 - i, _count: { members: 10 } })
      );
    ...
    it('gives correct rewards for rank 1', async () => {
      const guilds = [createMockGuild({ id: 'guild-1', honor: 2000, _count: { members: 10 } })];
    ...
    it('gives correct rewards for rank 2', async () => {
      const guilds = [
        createMockGuild({ id: 'guild-1', honor: 2000, _count: { members: 10 } }),
        createMockGuild({ id: 'guild-2', honor: 1800, _count: { members: 10 } }),
      ];
    ...
    it('gives correct rewards for rank 3', async () => {
      const guilds = Array.from({ length: 3 }, (_, i) =>
        createMockGuild({ id: `guild-${i}`, honor: 2000 - i * 100, _count: { members: 10 } })
      );
    ...
    it('awards champions trophy to top 10', async () => {
      const guilds = Array.from({ length: 15 }, (_, i) =>
        createMockGuild({ id: `guild-${i}`, honor: 2000 - i * 50, _count: { members: 10 } })
      );
  ...
  describe('invalidateLeaderboardCache', () => {
    it('deletes cache for specified week', async () => {
      ...
      expect(mockRedisDel).toHaveBeenCalledWith('leaderboard:guild:2025-W05:full');
    });
    ...
    it('uses current week by default', async () => {
      ...
      expect(mockRedisDel).toHaveBeenCalledWith('leaderboard:guild:2025-W10:full');
    });
  });
