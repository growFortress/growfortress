...
const websocketPlugin: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.decorate('wsBroadcastToUser', (userId: string, event: ServerEvent) => {
    // Send locally
    ...
    redis.publish(REDIS_WS_CHANNEL, JSON.stringify({
      ...
      fastify.log.error({ err }, 'Failed to publish WebSocket event to Redis');
    });
  });

  fastify.decorate('wsBroadcastToUsers', (userIds: string[], event: ServerEvent) => {
    // Send locally
  ...
  subscriber.on('message', (channel, message) => {
    ...
    try {
      ...
      fastify.log.error({ err, message }, 'Failed to process Redis WebSocket message');
    }
  });
  ...
  fastify.get('/ws', { websocket: true, config: { public: true } }, async (socket, request) => {
